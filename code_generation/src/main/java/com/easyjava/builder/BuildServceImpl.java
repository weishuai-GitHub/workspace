...
public class BuildServceImpl {
    ...
    public static void execute(TableInfo tableInfo) {
        ...
        try {
            ...
            bw.write("import " + Constants.get_package_mappers() + "." + tableInfo.getBeanName() + Constants.SUFFIX_MAPPERS +";\n\n");
            bw.write("import org.springframework.stereotype.Service;\n");
            bw.write("import javax.annotation.Resource;\n\n");
            BuildComment.createComment(bw, tableInfo.getTableComment() +"Service实现类");
            bw.write("@Service(\""+ tableInfo.getBeanName().substring(0,1).toLowerCase() +tableInfo.getBeanName().substring(1) +"Service\")\n");
            bw.write("public class " + className + " implements "+ tableInfo.getBeanName() +"Service {\n\n");

            bw.write("\t@Resource\n");
            String mapperName = tableInfo.getBeanName().substring(0,1).toLowerCase() +tableInfo.getBeanName().substring(1) +Constants.SUFFIX_MAPPERS;
            bw.write("\tprivate "+ tableInfo.getBeanName() + Constants.SUFFIX_MAPPERS+"<" + tableInfo.getBeanName()+","+tableInfo.getBeanParaName()+"> "
            + mapperName+";\n\n");
            ...
            for (Map.Entry<String, List<FieldInfo>> entry : keyIndexMap.entrySet()) {
                ...
                + methodParam + " ){\n");
                bw.write("        return this."+mapperName+".selectBy" + sb.substring(0, sb.length() - 3) + "(" + param + ");\n");
                bw.write("    }\n");
                bw.write("\n");
                ...
                bw.write("\t@Override\n");
                bw.write("    public Integer updateBy" + sb.substring(0, sb.length() - 3) + "( " + tableInfo.getBeanName() + " t,"
                + methodParam + " ){\n");
                bw.write("        return this."+mapperName+".updateBy" + sb.substring(0, sb.length() - 3) + "(t," + param + ");\n");
                bw.write("    }\n");
                bw.write("\n");
                ...
                bw.write("\t@Override\n");
                bw.write("    public Integer deleteBy" + sb.substring(0, sb.length() - 3) + "(" + methodParam + " ){\n");
                bw.write("        return this."+mapperName+".deleteBy" + sb.substring(0, sb.length() - 3) + "(" + param + ");\n");
                bw.write("    }\n");
                bw.write("\n");
            }
            bw.write("}\n");
