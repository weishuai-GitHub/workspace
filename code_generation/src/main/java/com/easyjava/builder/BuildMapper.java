        ...
        try {
            ...
            for(Map.Entry<String, List<FieldInfo>> entry : keyIndexMap.entrySet()) {
                ...
                for(FieldInfo key : keyList) {
                    methodParam.append(" @Param(\"").append(key.getPropertyName()).append("\") ").append(key.getJavaType()).append(" ").append(key.getPropertyName()).append(",");
                ...
                BuildComment.createFieldComment(bw,"根据"+sb.substring(0, sb.length()-3)+"查询"+tableInfo.getTableComment());
                bw.write("    T selectBy"+sb.substring(0, sb.length()-3)+"("+ methodParam + " );\n");
                bw.write("\n");
                BuildComment.createFieldComment(bw,"根据"+sb.substring(0, sb.length()-3)+"更新"+tableInfo.getTableComment());
                bw.write("    Integer updateBy"+sb.substring(0, sb.length()-3)+"( @Param(\"bean\") T t,"+ methodParam +" );\n");
                bw.write("\n");
                BuildComment.createFieldComment(bw,"根据"+sb.substring(0, sb.length()-3)+"删除"+tableInfo.getTableComment());
